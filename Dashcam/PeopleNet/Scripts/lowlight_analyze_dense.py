...
def main():
	if len(sys.argv) < 7:
		print("Usage: python3 lowlight_analyze_dense.py <sample_list.txt> <model.onnx> <out_dir> <conf> <step_seconds> <duration_seconds> [gamma_if_dark] [dark_thresh]")
		sys.exit(1)
	sample_list, model_path, out_dir = sys.argv[1], sys.argv[2], sys.argv[3]
	conf = float(sys.argv[4])
	step = float(sys.argv[5])
	duration = float(sys.argv[6])
	gamma_if_dark = float(sys.argv[7]) if len(sys.argv) > 7 else 1.8
	dark_thresh = float(sys.argv[8]) if len(sys.argv) > 8 else 30.0
	ensure_dir(out_dir)
	...
	report = []
	# Limit to first 8 for quick loop; adjust by env MAX_VIDEOS if provided
	try:
	...
	for vp in videos:
		...
		end_s = min(duration, (total / fps) if fps > 0 else duration)
		t = 0.0
		idx = 0
		while t <= end_s:
			frame_idx = int(t * fps) if fps > 0 else idx
			...
			# Save annotated frame when detection exists, else every 2s for quick scan
			save = len(dets) > 0 or (int(t*10) % 20 == 0)
			if save:
				annot = draw_dets(img_for_infer, dets)
				cv2.imwrite(os.path.join(vo, f"t{int(round(t*1000)):06d}ms_f{frame_idx}.jpg"), annot)
			report.append({
				"video": vp,
				"t_s": round(t, 3),
				"frame_idx": frame_idx,
			...
			idx += 1
		cap.release()
	with open(os.path.join(out_dir, "summary_dense.json"), "w") as f:
		json.dump(report, f, indent=2)
	print("Wrote:", os.path.join(out_dir, "summary_dense.json"))
